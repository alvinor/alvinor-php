...
class Logger
{
    ...
    private static $destination = 'storage/log';
    ...
    public static function getInstance()
    {
        if (! (self::$instance instanceof self)) {
            ...
            self::$destination = config('log.path', self::$destination);
            if (substr(self::$destination, 0, 1) != '/') {
                self::$destination = dirname(__DIR__) . '/' . self::$destination;
                if (! is_dir(self::$destination)) {
                    try {
                        mkdir(self::$destination);
                    } catch (\Exception $e) {
                        throw $e->getFile() . ':' . $e->getLine() . ':' . $e->getMessage();
                    }
                }
            }
        }
